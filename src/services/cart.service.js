 ...
 */
 const checkout = async (user) => {
  const cart = await Cart.findOne({email : user.email});
    ...
    if(cart == null){
      throw new ApiError(httpStatus.NOT_FOUND, "User does not have a cart. Use POST to create cart and add a product.")
    }
    ...
    if(!hasSetNonDefaultAddress){
      throw new ApiError(httpStatus.BAD_REQUEST, "Address cannot be empty");
    }
    ...
    if(user.walletMoney < total){
      throw new ApiError(httpStatus.BAD_REQUEST, "Balance inssfficient")
    }
